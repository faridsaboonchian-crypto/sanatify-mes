...
**۲) بوت‌استرپ اولین ادمین** (بن‌بست «VM تازه = هیچ‌کس لاگین نمی‌کند» را می‌بندد):
```bash
# روی VM — کنار exe:
./sanatify-mes --create-admin=admin:رمز-اولیه-۸کاراکتر-حرف‌ورقم
```
> - فقط وقتی **هیچ ادمین فعالی** وجود دارد کار می‌کند (گارد ادمین دوم — روی نصب زنده خطا می‌دهد).
...
**۴) ساخت بقیهٔ کاربران از UI:** ورود با ادمین بوت‌استرپ → تغییر اجباری رمز → تب **org** → کاربران و نقش‌ها (اپراتور، سرپرست، برنامه‌ریز، کنترل کیفیت، …).
