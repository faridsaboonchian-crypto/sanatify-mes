...
fs.writeFileSync(SP + '.bak_p32_3', s);
fs.writeFileSync(HP + '.bak_p32_3', h);
...
function replaceFirst(src, oldStr, newStr, name) {
    ...
    if (idx === -1) { log.push(name + ': ANCHOR MISSING at position search'); ok = false; return src; }
    log.push(name + ': replaced at position ' + idx);
    return src.slice(0, idx) + newStr + src.slice(idx + oldStr.length);
}

// ====================================================================
// SERVER: S1 - اضافه کردن heat_number به production
// ====================================================================
...
source: 'manual',
timestamp: new Date().toISOString(),`;
s = replaceFirst(s, OLD_PROD, NEW_PROD, 'S1-production-heat');
...
// ====================================================================
const OLD_BODY = `machine_id: 'st-pack', description: document.getElementById('fpDesc').value.trim() };`;
const NEW_BODY = `machine_id: 'st-pack', heat_number: document.getElementById('fpHeat').value.trim(), description: document.getElementById('fpDesc').value.trim() };`;
h = replaceFirst(h, OLD_BODY, NEW_BODY, 'H2-prod-send-heat');
...
h = replaceFirst(h, OLD_CLEAR, NEW_CLEAR, 'H3-prod-clear-heat');
...
s = replaceFirst(s, OLD_GENE, NEW_GENE, 'S2-genealogy-extended');
...
s = replaceFirst(s, OLD_CONS, NEW_CONS, 'S3-consume-gin-link');
...
s = replaceFirst(s, OLD_LOG, NEW_LOG, 'S4-consume-log-link');
